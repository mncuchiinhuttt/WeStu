...
export async function shareTest (interaction: any) {
	const languageService = LanguageService.getInstance();
	const userLang = await languageService.getUserLanguage(interaction.user.id);
	const langStrings = require(`../../../data/languages/${userLang}.json`);
	const strings = langStrings.commands.flashcard.shareTest;
    ...
    if (sharedGroups?.groupIds.includes(group_id)) {
      const testAlreadyShared_embed = new EmbedBuilder()
        .setTitle(strings.testAlreadyShared)
        .setColor('#FF0000')
        .setTimestamp(Date.now());
      await interaction.reply({
        embeds: [testAlreadyShared_embed],
        ephemeral: true
      });
      return;
    }

    const test = await TestService.shareTest(
      your_test_id,
      group_id
    );
    ...
    const fetchTest = await Test.findById(your_test_id);
      ...
      .setTitle(strings.testShared)
      .setDescription(
        strings.testSharedDescription
    ...
    const DM_embed = new EmbedBuilder()
      .setTitle(strings.testShared)
      .setDescription(
        strings.testSharedDescriptionDM
          .replace('{name}', fetchTest?.title)
          .replace('{group}', fetchGroup?.name)
